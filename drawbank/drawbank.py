...
def parse_assembly(fnames):
    ...
    if nb_no_date > 0:
        logging.getLogger().warning(
            f"{nb_no_date} genomes had no date of submission in your assembly summary."
        )
    return n_spe, years
...
def make_urls(section, taxons):
    ...
    for tax in taxons:
        if tax == "all":
            urls.add(
                (
                    f"{uri}/{section}/assembly_summary_{section}.txt",
                    f"{CACHE_DIR}/{section}_assembly_summary.txt",
                )
            )
        else:
            urls.add(
                (
                    f"{uri}/{section}/{tax}/assembly_summary.txt",
                    f"{CACHE_DIR}/{section}_{tax}_assembly_summary.txt",
                )
            )
    return urls
...
def get_taxonomic_groups(groups):
    ...
    for tax in taxons:
        if tax not in TAXONOMIC_GROUPS:
            raise Exception(
                f"{tax} is not a known subdivision of genbank (or refseq). Please use any of {','.join(TAXONOMIC_GROUPS)}"
            )
...
def cmdline():
    ...
    parser = argparse.ArgumentParser(
        description="Draws the current number of genomes in GenBank, with the given filters",
    ...
    fig.add_argument(
        ...
        type=int,
        default=5,
        help="Indicate the n most numerous group",
    )
    fig.add_argument(
        "-c",
        "--cumulative",
        ...
        required=False,
        action="store_true",
        help="make a cumulative bar chart rather than have each year's addition",
    )
    ...
    dat.add_argument(
        ...
        type=str.lower,
        default="all",
        help=f"comma separated list of groups to download from (any value among {','.join(TAXONOMIC_GROUPS)})",
    )
    dat.add_argument(
        "-N",
        "--no_cache",
        ...
        action="store_true",
        default=False,
        help="Do not cache downloaded files (nor use cached files).",
    )
    ...
    misc.add_argument(
        ...
        required=False,
        type=str,
        help="An assembly summary to use (if not provided, will download the latest one automatically)",
    )
    misc.add_argument(
        "--verbose",
        required=False,
...
def main():
    ...
    logging.getLogger().info(
        "Drawing is done. Figure should be opened in your system's default browser."
    )
